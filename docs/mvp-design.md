...
- **17 列目の `metadata jsonb` は 2026-07-08 に実装済み**(スキーマは §4 参照。
  全行同値、JSON は PL/R 内で base R により手組み — 実行環境に jsonlite がないため)。
...
DISTINCT で全行同値を検証、数値のみモデルで空オブジェクトを確認)。既存テストは
予測通り expected 無変更で通過した。JSON は PL/R 内で base R により手組みしている
(実行環境に jsonlite がないため。jsonb がキー順を正規化するので出力は決定的)。
